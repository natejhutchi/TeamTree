...
- Revisit the muted bracketed aside on the `I am not using AI` card. Current copy is too dense and needs a quicker, clearer version explaining that AI still needs designers to guide it, fix it, and keep it on-brand, so the work shifts into prompting and cleanup instead of disappearing.
...
The editor should handle normal text editing reliably: cursor placement, Enter/new lines, text selection, bold, italic, lists, indentation, alignment, colors, and undo behavior. TeamTownTrees-specific behavior should be layered on top.
